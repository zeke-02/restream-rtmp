    ...
    try{
        ...
        nms.on('postPublish',  async (id, StreamPath, args) => {
            ...
            if (StreamPath.indexOf('/stream/') != -1){
              ...
              if (args.youtube && !this.sessions.has(`youtube-${id}`)){
                const url = `rtmp://a.rtmp.youtube.com/live2/${args.youtube}`;
              ...
              if (args.facebook && !this.sessions.has(`facebook-${id}`)){
                const url = `rtmps://live-api-s.facebook.com:443/rtmp/${args.facebook}`;
                const session = nms.nodeRelaySession({
                  ffmpeg: ffmpeg_path,
                  inPath: `rtmp://127.0.0.1:${config.rtmp.port}${StreamPath}`,
                  ouPath: url
              ...
              if (args.twitch && !this.sessions.has(`twitch-${id}`)){
                const url = `rtmp://live.twitch.tv/app/${args.twitch}`;
                const session = nms.nodeRelaySession({
                  ffmpeg: ffmpeg_path,
                  inPath: `rtmp://127.0.0.1:${config.rtmp.port}${StreamPath}`,
                  ouPath: url,
